 ...
 *
 */
public class AnnotatedProgress extends Composite {
	...
	private LinkedList<ProgressAnnotation> items;
	private DisposeListener itemDisposedListener;
	private ProgressAnnotation selectedAnnotation;
	private AnnotatedProgressHelper helper;
	 ...
	 * @param style
	 */
	public AnnotatedProgress(Composite parent, int style) {
		super(parent, style);
		items = new LinkedList<ProgressAnnotation>();
		...
		helper = new AnnotatedProgressHelper(this);
	...
	protected void resetScale() {
		...
		scale = ((double)visualRangeSize)/(getMaximum());
	...
	private void paintRanges(GC gc, Rectangle bounds) {
		...
		gc.setAlpha(200);
		for (ProgressAnnotation a : items) {
		...
		if (selectedAnnotation != null) {
			ProgressAnnotation a = selectedAnnotation;
	 ...
	 */
	public ProgressAnnotation itemAt(Point p) {
		return itemAt(p, true);
	}
	
	protected ProgressAnnotation itemAt(Point p, boolean includeDisabled) {
		checkWidget();
		ProgressAnnotation winner = null;
		
		long score = -1;
		long rangeValue = toRangeValue(p.x);
		ProgressAnnotation[] annotationsToScore = itemsAt(p);
		for (ProgressAnnotation a : annotationsToScore) {
			if (a == null || a.isDisposed()) continue;
			if (!includeDisabled && !a.isEnabled()) continue;
			long localScore = 0;
	...
	protected ProgressAnnotation[] itemsAt(Point p) {
		//first, look for all the ones that are in the pixel range
		ProgressAnnotation[] currentRanges = items.toArray(new ProgressAnnotation[items.size()]);
		LinkedList<ProgressAnnotation> annotationsToScore = new LinkedList<ProgressAnnotation>();
		for (ProgressAnnotation a : currentRanges) {
		...
		return annotationsToScore.toArray(new ProgressAnnotation[annotationsToScore.size()]);
	}
	
	public ProgressAnnotation[] getRanges() {
		return items.toArray(new ProgressAnnotation[items.size()]);
	}
	...
	public void setSelectedMaximum(long rangeMax) {
		checkWidget();
		//don't do it if the user is trying to move
		//the slider
		if (helper.armed()) {
			return;
		}
...
//		}
		...
		return Math.round((x-bounds.x+HANDLE_SIZE)/scale);
	}
	...
	private void fireItemSelectionChanged(int button) {
		...
		event.data = selectedAnnotation.getData();
		notifyListeners(SWT.Selection, event);
	}
	 ...
	 */
	void createItem(ProgressAnnotation rangeAnnotation) {
	...
	void updateSelection(MouseEvent e) {
		Item item = itemAt(new Point(e.x, e.y), false);
		if (item != selectedAnnotation) {
			if (item instanceof ProgressAnnotation) {
				selectedAnnotation = (ProgressAnnotation) item;
			} else {
	 ...
	 * -1 if the item is disposed, or doesn't exist in this slider. 
	 */
	public int getIndex(ProgressAnnotation item) {
		checkWidget();
	 ...
	 * is out of range.
	 */
	public ProgressAnnotation getItem(int index) {
	...
	public boolean setSelectionIndex(int index) {
		...
		ProgressAnnotation newSelection = items.get(index);
