...
import net.sourceforge.tagsea.c.documents.internal.NewWaypointDefinitionExtractor;
import net.sourceforge.tagsea.c.resources.internal.WaypointResourceUtil;
...
class DocumentChangeOperation extends TagSEAOperation {
	...
	public IStatus run(IProgressMonitor monitor) throws InvocationTargetException {
		...
		if (file != null && file.exists()) {
			IWaypoint[] waypoints = CWaypointsPlugin.getCWaypointsForFile(file);
			boolean refreshRegion = true;
			List<IWaypoint> intersectingWaypoints = new LinkedList<IWaypoint>();

			for (IWaypoint waypoint : waypoints) {
				int offset = CWaypointUtils.getOffset(waypoint);
				int length = CWaypointUtils.getLength(waypoint);
				...
				if (event.fOffset > end || (end == event.fOffset && startsLine)) {
					...
					CWaypointUtils.setOffset(waypoint, offset);
					CWaypointUtils.setEnd(waypoint, offset+oldLength);
...
//						delete the waypoint
					...
					try {
						...
						IParsedCWaypointInfo info = parser.parse(text, rs[0].getOffset());
	...
	private IStatus updateForNew(IFile file) {
		...
		class WaypointComparator implements Comparator<IWaypoint> {
			public int compare(IWaypoint o1, IWaypoint o2) {
				int off1 = CWaypointUtils.getOffset(o1);
				int off2 = CWaypointUtils.getOffset(o2);
				int diff = off1-off2;
				if (diff == 0) {
					int l1 = CWaypointUtils.getLength(o1);
					int l2 = CWaypointUtils.getLength(o2);
					diff = l1-l2;
				}
				return diff;
			}
		...
		MultiStatus status = new MultiStatus(CWaypointsPlugin.PLUGIN_ID, IStatus.OK, "", null);
		...
		IWaypoint[] waypoints = CWaypointsPlugin.getCWaypointsForFile(file);
		for (IWaypoint waypoint : waypoints) {
			waypointRegions.add(new Region(CWaypointUtils.getOffset(waypoint), CWaypointUtils.getLength(waypoint)));
		}
	...
	private IStatus createWaypointForRegion(ICWaypointParser parser, IRegion region, IFile file) {
		try {
			...
			return new Status(IStatus.WARNING, CWaypointsPlugin.PLUGIN_ID,IStatus.WARNING, e.getMessage(), e);
		}
	 ...
	 * @param info
	 */
	private IStatus updateWaypoint(IWaypoint waypoint, IParsedCWaypointInfo info, IFile file) {
		...
		for (String newT : newTagsList) {
			...
			if (tag == null) {
				status.merge(new Status(IStatus.WARNING, CWaypointsPlugin.PLUGIN_ID, IStatus.WARNING, "Could not add tag " + newT, null));
			}
		}
		
		status.merge(CWaypointUtils.setOffset(waypoint, info.getOffset()).getStatus());
		status.merge(CWaypointUtils.setEnd(waypoint, info.getOffset() + info.getWaypointData().length()).getStatus());
	...
	private void removeProblems(IRegion region, IFile file) throws CoreException {
		IMarker[] markers = file.findMarkers(ICWaypointsConstants.WAYPOINT_PROBLEM_MARKER, false, IResource.DEPTH_ZERO);
