...
public class WaypointFilterUI implements IWaypointFilterUI {
	...
	public void applyToFilter() {
		IPreferenceStore store = CWaypointsPlugin.getDefault().getPreferenceStore();
		if (anyResourceButton.getSelection()) {
			store.setValue(CWaypointPreferences.FILTER_PREFERENCE_KEY, CWaypointPreferences.FILTER_ANY);
		} else if (anyResourceInSameProjectButton.getSelection()){
			store.setValue(CWaypointPreferences.FILTER_PREFERENCE_KEY, CWaypointPreferences.FILTER_PROJECT);
		} else if (selectedResourceButton.getSelection()) {
			store.setValue(CWaypointPreferences.FILTER_PREFERENCE_KEY, CWaypointPreferences.FILTER_SELECTED);
		} else if (selectedResourceAndChildrenButton.getSelection()) {
			store.setValue(CWaypointPreferences.FILTER_PREFERENCE_KEY, CWaypointPreferences.FILTER_CHILDREN);
		}
		store.setValue(CWaypointPreferences.LINK_TO_RESOURCES_KEY, linkToResourceButton.getSelection());
	}
	...
	public Control createControl(Composite parent) {
		...
		String filterType = CWaypointPreferences.getCurrentFilterType();
		if (filterType.equals(CWaypointPreferences.FILTER_ANY)) {
			anyResourceButton.setSelection(true);
		} else if (filterType.equals(CWaypointPreferences.FILTER_CHILDREN)) {
			selectedResourceAndChildrenButton.setSelection(true);
		} else if (filterType.equals(CWaypointPreferences.FILTER_PROJECT)) {
			anyResourceInSameProjectButton.setSelection(true);
		} else if (filterType.equals(CWaypointPreferences.FILTER_SELECTED)) {
			selectedResourceButton.setSelection(true);
		}
		if (CWaypointPreferences.isFilterLinkedToResources()) {
