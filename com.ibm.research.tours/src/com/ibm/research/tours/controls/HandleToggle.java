 ...
 */
public class HandleToggle extends Composite {
	 ...
	 * @see SWT#VERTICAL
	 */
	public HandleToggle(Composite parent, int style) 
	{
	...
	protected void createControl() 
	{
		...
		addPaintListener(new PaintListener() {		
			public void paintControl(PaintEvent e) {
				...
				gc.fillRectangle(0,0,ROUNDED_RADIUS/2,ROUNDED_RADIUS/2);
				gc.setBackground(getBackground());
				gc.fillArc(0, 0, ROUNDED_RADIUS, ROUNDED_RADIUS, 90, 90);
				
				int xOffset = 2; 
				int startDrawing = (bounds.height / 2) - ((DOT_NUMBER / 2) * DOT_DISTANCE) - DOT_DISTANCE;
				for (int i = 0; i < DOT_NUMBER; i++)
				{
					gc.setBackground(white);
					gc.fillRectangle(xOffset+2, startDrawing + 1 + (i * DOT_DISTANCE), 2, 2);					
					gc.setBackground(grey);
					gc.fillRectangle(xOffset+1, startDrawing + (i * DOT_DISTANCE), 2, 2);	
				}
				
				gc.setForeground(fLineColor);
				gc.drawLine(0, ROUNDED_RADIUS/2, 0, bounds.height);
				gc.drawArc(0, 0, ROUNDED_RADIUS, ROUNDED_RADIUS, 90, 90);
