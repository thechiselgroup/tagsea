...
$lockfileName = "$root/.lock-january2008";
$logFile =    "$root/uploadLog-january2008.txt";
$uidMapFile = "$root/tagSEAUidMap-january2008.txt";

$dir = "$root/january2008"; 
...
foreach(keys %FORM){
	...
	# This must be TAGSEAa where is a a number
	if($_ =~ /^TAGSEA(\d+)/){
		$uid = $1;
		if(&checkUID($uid) == 1)
		{
