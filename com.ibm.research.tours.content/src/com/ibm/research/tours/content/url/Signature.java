...
public static String toPortableString(String methodSignature, String methodName,boolean fullyQualifyTypeNames, boolean isVargArgs) 
{
	char[] methodSignatureArray = methodSignature.toCharArray();
	
	int firstParen = CharOperation.indexOf(C_PARAM_START, methodSignatureArray);
	if (firstParen == -1) {
		throw new IllegalArgumentException();
	}
	
	StringBuffer buffer = new StringBuffer(methodSignatureArray.length + 10);
	...
	// parameters
	buffer.append('(');
	char[][] pts = getParameterTypes(methodSignatureArray);
	for (int i = 0, max = pts.length; i < max; i++) {
		if (i == max - 1) {
			appendTypeSignature(pts[i], 0 , fullyQualifyTypeNames, buffer, isVargArgs);
		} else {
	...
	// return type
	char[] rts = getReturnType(methodSignatureArray);
	appendTypeSignature(rts, 0 , fullyQualifyTypeNames, buffer);
