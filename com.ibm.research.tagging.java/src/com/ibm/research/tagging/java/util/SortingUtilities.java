...
{
	public static void sortRegionsByAscendingOffset(List<IRegion> regions)
	{
	...
	public static void sortWaypointsByDescendingOffset(List<JavaWaypoint> waypoints)
	{
		if(waypoints.size() > 1)
		{
			Collections.sort(waypoints,new Comparator<JavaWaypoint>() 
			{
				public int compare(JavaWaypoint a, JavaWaypoint b) 
				{
					int aOffset = MarkerUtilities.getCharStart(a.getMarker());
					int bOffset = MarkerUtilities.getCharStart(b.getMarker());
	...
	public static Map<IResource, List<JavaWaypoint>> sortWaypointsByResource(IWaypoint[] waypoints)
	{
