...
import com.ibm.research.tagging.core.ui.expressions.ExpressionMatcher;
 ...
 * @since 3.2
 */
public class ExpressionPatternFilter extends AbstractPatternFilter
{    
	static final Pattern EXPRESSION_PATTERN = Pattern.compile("(\\w+)([<>=!])(\\S+)\\*"),
						 TAG_PATTERN        = Pattern.compile("(\\@)(\\S)(\\S+)");
    ...
    public void setPattern(String patternString) 
    {
		...
		{			
			...
			if(words.length > 0)
			{
				...
				{
					Matcher m = EXPRESSION_PATTERN.matcher(word.toLowerCase());
					if ( m.matches() )
					{
						...
						if ( !right.equals("*") )
						{
							ExpressionMatcher matcher = ExpressionMatcherFactory.create(left, middle.charAt(0), right);
							if ( matcher!=null )
							{
					...
					{
						Matcher m2 = TAG_PATTERN.matcher(word.toLowerCase());
						if ( m2.matches() )
						{
							String left    = m2.group(1),
							       middle  = m2.group(2),
							       right   = m2.group(3);
    ...
    public boolean isLeafMatch(Viewer viewer, Object element)
    {
        ...
        if (expressionMatchers!=null && expressionMatchers.size()>0)
        {
    ...
    /**
     * take given object and try to match with expression matchers
     */
    private boolean expressionMatches(Object element) {
    	boolean result = false; 
    		
    	for (int i=0; i<expressionMatchers.size(); i++)
    	{
    		ExpressionMatcher matcher = expressionMatchers.get(i);
