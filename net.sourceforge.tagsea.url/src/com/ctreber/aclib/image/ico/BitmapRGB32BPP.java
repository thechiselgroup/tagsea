...
// Source File Name:   BitmapRGB32BPP.java
...
public class BitmapRGB32BPP extends AbstractBitmapRGB
{
    ...
    public BitmapRGB32BPP(BitmapDescriptor bitmapdescriptor)
    {
        ...
        throws IOException
    {
        ...
        {
            byte abyte0[] = abstractdecoder.readBytes(getWidth() * 4, null);
            int j = 0;
            int k = (getHeight() - i - 1) * getWidth();
            for(int l = 0; l < getWidth(); l++)
                _samples[k++] = abyte0[j++] + (abyte0[j++] << 8) + (abyte0[j++] << 16) + (abyte0[j++] << 24);
