 ...
 */
@Deprecated
public class WaypointDefinitionExtractor 
{
	...
	protected static final String META_OPEN_STRING = "["; //$NON-NLS-1$
	@Deprecated
	protected static final char META_CLOSE_CHAR = ']';
	@Deprecated
	protected static final String META_CLOSE_STRING = "]"; //$NON-NLS-1$
	/*
...
//	protected static final Object TAG_META_CLOSE_KEY = new Object();
	...
	static
	{
		...
		WordRule tagRule = new WordRule(new WaypointDetector());
...
//		metaCloseRule.addWord(META_CLOSE_STRING, (IToken)new Token(TAG_META_CLOSE_KEY));
		...
		fWaypointScanner.setRules(new IRule[]{tagRule,/*metaOpenRule,metaCloseRule,*/eolRule,eocRule});
	}
...
//		boolean metaOpenDetected = false;
		...
		while (true) 
		{
			...
			else if(token.getData() == EOL_KEY) 
			{
				...
				{
					eolOffset = fWaypointScanner.getTokenOffset();
					tagRegions.add(new Region(tagOffset, eolOffset - tagOffset-1));
					tagDetected = false;
				}
			}
//			else if(token.getData() == TAG_META_OPEN_KEY) 
