...
public class WaypointFilter implements IWaypointFilter {
	...
	public boolean select(IWaypoint waypoint) {
		String type = JavaWaypointPreferences.getCurrentFilterType();
		if (type == JavaWaypointPreferences.FILTER_ANY) {
			return true;
		}
		...
		if (type.equals(JavaWaypointPreferences.FILTER_CHILDREN)) {
        	return isSelectedChild(waypoint);
        } else if (type.equals(JavaWaypointPreferences.FILTER_PROJECT)) {
        	return isSelectedProject(waypoint);
        } else if (type.equals(JavaWaypointPreferences.FILTER_SELECTED)) {
