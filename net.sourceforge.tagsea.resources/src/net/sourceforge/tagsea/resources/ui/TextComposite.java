...
public class TextComposite extends Composite {
	...
	private Text regionValueLabel = null;
	private Label selectionLabel = null;
	private Text fileValueLabel = null;
	...
	public TextComposite(Composite parent, int style, String fileName, IRegion region, String text) 
	{
		super(parent, style);
		fFileName = fileName;
		fRegion = region;
	...
	private void createDetailsGroup() {
		...
		fileLabel = new Label(detailsGroup, SWT.NONE);
		fileLabel.setText("File");
		
		fileValueLabel = new Text(detailsGroup, SWT.BORDER);
		fileValueLabel.setEditable(false);
		fileValueLabel.setBackground(Display.getDefault().getSystemColor(SWT.COLOR_INFO_BACKGROUND));
		fileValueLabel.setText(fFileName);
		
		GridData data = new GridData(GridData.FILL_HORIZONTAL);
		data.widthHint = 350; //This will expand the dialog
		fileValueLabel.setLayoutData(data);

		regionLabel = new Label(detailsGroup, SWT.NONE);
		regionLabel.setText("Start/End");
		
		regionValueLabel = new Text(detailsGroup, SWT.BORDER); 
		regionValueLabel.setEditable(false);
		regionValueLabel.setBackground(Display.getDefault().getSystemColor(SWT.COLOR_INFO_BACKGROUND));
		regionValueLabel.setText(fRegion.getOffset() + "/" + (fRegion.getOffset() + fRegion.getLength()));
		
		data = new GridData(GridData.FILL_HORIZONTAL);
		data.horizontalSpan = 2;
		selectionLabel = new Label(detailsGroup, SWT.NONE);
		...
		ScrolledComposite sc = new ScrolledComposite(detailsGroup,SWT.BORDER| SWT.H_SCROLL | SWT.V_SCROLL);
		fViewer= new SourceViewer(sc,null,SWT.NONE);
		fViewer.configure(new SourceViewerConfiguration());
		fViewer.setEditable(false);
		IDocument doc= new Document(fText);
		fViewer.setInput(doc);
		//fViewer.getControl().setLayoutData(data);
		fViewer.getTextWidget().setBackground(Display.getDefault().getSystemColor(SWT.COLOR_INFO_BACKGROUND));
		fViewer.getTextWidget().setForeground(Display.getDefault().getSystemColor(SWT.COLOR_INFO_FOREGROUND));
		//fViewer.getTextWidget().setFont(JFaceResources.getTextFont());
		sc.setContent(fViewer.getControl());
	...
	private void createDateComposite() {
		...
		dateButton.addSelectionListener(new SelectionListener(){
			...
			public void widgetSelected(SelectionEvent e) {
				...
				int result = dialog.open();
				if (result == Dialog.OK) {
					TextComposite.this.date = dialog.getDate();
					DateFormat format = DateFormat.getDateInstance(DateFormat.MEDIUM);
					dateValueLabel.setText(format.format(date));
				}
			}
